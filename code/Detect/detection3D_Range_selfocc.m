function [bbw,bbs_3d_f,occCount,match,searchRage,occCountAll] =detection3D_Range_selfocc(feature,num_dect,Space,Model,pointCountIntegral,occSource,Modelocc,Localsearch,pad,removedontcare,featurepara)
        svm =Model.svm;
        Cdepth = Model.Cdepth;
        ...
        if  exist('Cx','var')&&~isempty(Cx)&&Localsearch
            ...
            if featurepara.Localview
               ...
               %}
               %{
               if Cx + size(svm.w,2)*0.1<0,
                   range_f_xmax = min(round(cameraLine+0.3*size(svm.w,2)),range_f_xmax);
               else
                   range_f_xmin = max(round(cameraLine-0.3*size(svm.w,2))-size(svm.w,2),range_f_xmin);
               end
               if Cx - size(svm.w,2)*0.1>0,
                   range_f_xmin = max(round(cameraLine-0.3*size(svm.w,2)),range_f_xmin);
               else
                   range_f_xmax = min(round(cameraLine+0.3*size(svm.w,2))+size(svm.w,2),range_f_xmax);
               end
               %}
        ...
        [match,occCountAll] = evaluate_score_4d_flag(featuresearch,svm.w,bias,pointCountIntegral,occSource,Modelocc,careMask,[Space.s,SpaceRangeDepth],featurepara);
        ...
        searchRage =struct('range_f_xmin',range_f_xmin,'range_f_dmin',range_f_dmin,'range_f_hmin',range_f_hmin);
end

function [match,occCount] = evaluate_score_4d_flag(featureM,templateM,bias, pointCountIntegral,occSource,Modelocc,careMask,SpaceRange,featurepara)
...
if isfield(featurepara,'occcellmaxratio')
   Occthreshold = round(featurepara.occcellmaxratio*sum(careMask(:)));
   [match,occCount] = fconv3d_handle3(featureM, templateM, occSource, Modelocc>0,EmptyBox,~careMask,SpaceRange,featurepara.scalemissing,Occthreshold);
else
   [match,occCount] = fconv3d_handle2(featureM, templateM, occSource, Modelocc>0,EmptyBox,~careMask,SpaceRange,featurepara.scalemissing);
end
match =match+bias;
end