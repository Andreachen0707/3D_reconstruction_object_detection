//mex -largeArrayDims CFLAGS='-O3 -ffast-math -std=c++11' fconv3d_handle3.cc
#include "mex.h"
#include <math.h>
#include <string.h>

// convolve A and B
void process(double* A, double* B, double* MA, bool* MB, bool* E, bool* DontCare, double* C, const mwSize* A_dims, const mwSize* B_dims, const mwSize* C_dims, const double* Space, const double* scalemissing,double* occCount, const double* Occthreshold) {
	double* dst = C;
	bool* isE = E;
	double* ptr_occCount = occCount;
//	int count =0;
		...
		total = total+1;
								...
								if ((*MD_off_y)){
									...
									if ((*MA_off_y)>0.001 && !(*MB_off_y) && (y*Space[0] +Space[1]> (*MA_off_y))) {
										occludednum +=1;
										// this cell is occluded by souce outside box
										if  (occludednum<Occthreshold[0]){
											// is still not too much are occluded 
											val += *(B_off_y+B_dims[0]-1)*scalemissing[0];
											//printf("Occthreshold: %f,occludednum: %f total :%f\n",Occthreshold[0],occludednum,total);
										}
										else{
...
void mexFunction(int nlhs, mxArray *plhs[], int nrhs, const mxArray *prhs[]) { 
	...
	const mxArray *mxOccthreshold = prhs[8];
	...
	double* Occthreshold = (double*)mxGetPr(mxOccthreshold);
	...
	plhs[1] = mxCreateNumericArray(3, C_dims, mxDOUBLE_CLASS, mxREAL);
	double* C = (double*)mxGetPr(plhs[0]);
	double* occCount = (double*)mxGetPr(plhs[1]);

	// do convolutions
	process(A, B, MA, MB, E, DontCare, C, A_dims, B_dims, C_dims,Space,scalemissing,occCount,Occthreshold);
}
