function [bbw,bbs_3d_f,match] =detection3D_Range_tree(feature,num_dect,Space,Model,pointCountIntegral,occSource,Modelocc,Localsearch,pad,removedontcare,featurepara)
        Cdepth = Model.Cdepth;
        ...
        tic;
        match = evaluate_score_4d_flag(featuresearch,Model,pointCountIntegral,occSource,Modelocc,careMask,[Space.s,SpaceRangeDepth],featurepara);
        toc;
...
function match = evaluate_score_4d_flag(featureM,Model, pointCountIntegral,occSource,Modelocc,careMask,SpaceRange,featurepara)
careMask= repmat(careMask,[1,1,1,size(featureM,4)]);
    ...
    dim3 = dim3(~EmptyBox);
    for k = 1:length(dim1(:)),
         boxfea=getTemplate_3d(featureM,[dim2(k),dim1(k),dim3(k),sizeTemplate(2),sizeTemplate(1),sizeTemplate(3)]); 
         if   isfield(featurepara,'kernelsvm')&&featurepara.kernelsvm,
            [~,~,match(dim1(k),dim2(k),dim3(k))] =svmpredict(0,boxfea(careMask>0)',Model.SVMmodel);
         else
             match(dim1(k),dim2(k),dim3(k)) =predict(Model.B,boxfea(careMask>0)');
         end
    end
end